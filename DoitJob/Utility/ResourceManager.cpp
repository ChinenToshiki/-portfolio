...
const std::vector<int>& ResourceManager::GetImageResource(const std::string& file_name, int all_num, int num_x, int num_y, int size_x, int size_y)
{
	...
	auto file_path = "Resource/Images/" + file_name;
	...
	if (image_container.count(file_path) == NULL)
	{
		...
		{
			// 分割読み込みを行う
			CreateImagesResource(file_path, all_num, num_x, num_y, size_x, size_y);
		}
		if (all_num == 1)
		{
			// 分割読み込みを行わない
			CreateImageResource(file_path);
		}
...
const int& ResourceManager::GetSoundResource(const std::string& file_name)
{
	auto file_path = "Resource/Sounds/" + file_name;
